...
> = {
  ...
  twitter: {
    ...
    glowColor: "rgba(29, 161, 242, 0.25)",
  },
  whatsapp: {
                  ...
                  return (
                    <li key={social.id}>
                      <motion.a
                        ...
                        title={social.label}
                        className="group relative flex h-10 w-10 items-center justify-center rounded-xl bg-background/80 border border-border/60 text-muted-foreground transition-all duration-300 hover:border-transparent shadow-xs"
                        whileHover={{
                          scale: 1.15,
                          y: -3,
                          boxShadow: `0 8px 20px -4px ${iconConfig.glowColor}`,
                        }}
                        whileTap={{ scale: 0.95 }}
                        transition={{ type: "spring", stiffness: 400, damping: 17 }}
                      >
                        <svg
                          className={`h-5 w-5 transition-colors duration-300 group-hover:${iconConfig.hoverColor}`}
                          fill="currentColor"
